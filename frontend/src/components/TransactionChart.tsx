...
export default function TransactionChart({ transactions }: { transactions: Transaction[] }) {
  ...
  const totalData = [
    {
      name: 'Income',
      value: transactions.filter(tx => tx.type === 'income').reduce((sum, tx) => sum + tx.amount, 0)
    },
    {
      name: 'Expense',
      value: transactions.filter(tx => tx.type === 'expense').reduce((sum, tx) => sum + tx.amount, 0)
    }
  ]
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <PieChart>
            <Pie data={totalData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100}>
